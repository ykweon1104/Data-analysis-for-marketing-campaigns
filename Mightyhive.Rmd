...
```
Draw charts and maps to target areas for marketing campaigns
```{r}
...
mod = glm(transactions~1., data=dat_cali_organic2) #Null model
f_mod = glm(transactions~., data=dat_cali_organic2) #Full model
best_model <- step(object = f_mod, scope = list(upper = mod), direction = "both", trace = TRUE) #get the best model

dat_cali_organic3<- dat_cali_organic %>% dplyr::select(transactions, bounces, hits, pageviews, timeOnSite, source, deviceCategory) #Select the best model
